...
| **核心空头逻辑** | **价格涨 24 倍而采用度只涨 2.6 倍**；屏蔽占比未回到峰值；开发基金年支出约 $9,515 万；隐私币的监管风险不可量化 |
...
## 4. 一次可以被完整测量的隐私事件（本期核心章之一）

> **前十期从来没有过这样的样本**：一个有确切日期的冲击，
...
> **本报告倾向于前者**（理由：价格同期 –37.2%，说明反应是广泛的），
> **但这是一个间接论证，不是对集中度的直接测量。已登记为记分卡的下期必查项。**
>
...
| 观测 | 值 |
|------|---:|
| **峰值到谷底（04-25 → 07-12）屏蔽占比** | **31.01% → 25.49%，–5.52 个百分点** |
| **Orchard 峰值到披露后（04-25 → 07-04）** | **4,547,641 → 3,765,676，–78.2 万 ZEC** |

> **这是本系列第一次能把一个信任冲击量化成「多少资金用脚投票、什么时候投的」。**
> ⚠️ **注意措辞**：能测到的是**资金量**，不是**用户数**——
> **屏蔽池的性质决定了本报告永远无法知道这背后是 10 个地址还是 10,000 个。**
>
> ⚠️ **但要说清这条曲线证明不了什么**：
...
> **本报告倾向于②，但把握不高（约 55%）**，理由是迁移在 8 月才完成大部分、恢复曲线仍在上行。
> **已按门槛 8 登记为记分卡 Z8-1 的反向论点。**
>
...
| # | 论据 | 强度 | 说明与证据等级 |
|---|------|:---:|------|
| 1 | **屏蔽占比在冲击后已恢复约一半**（25.49% → 28.85%） | ★★★★☆ | ✅ **节点 `valuePools` 逐块直读**，22 个时点。⚠️ **降为四星并加注**：**本条与空头 #1 是同一个数字（28.85%）的两面**，不是两条独立证据——见 8.2 说明 |
...
| # | 论据 | 强度 | 说明与证据等级 |
|---|------|:---:|------|
| 1 | **价格创新高，而屏蔽占比仍低于 4 月峰值 2.16pp** | ★★★☆☆ | ✅ 两侧均一手。⚠️ **降为三星**：**本条与多头 #1 共用同一个观测（28.85%）**——多头读「已恢复 +2.76pp」，空头读「仍差 –2.16pp」。**两者都对，但它们是同一件事，读者不应把它们计为两条独立证据。**⚠️ 比率与价格的倍数不可直接相比（见 7.1 限定③） |
...
3. **漏洞事件给出了一个可复用的压力测试基线。**
   **下一次隐私资产出现信任冲击时，可以用这次的数字做参照**：
   披露后 3 天撤出 26 万枚、峰谷 –5.52pp、两个月恢复约一半。
...
本期能把「信任冲击」量化成 26 万 ZEC 和 4.92 个百分点，**是因为恰好有一个干净的指标和一个确切的日期。**
...
| 数据 | 来源 | 核实状态 |
|------|------|:---:|
| **六个池的余额与逐块 delta** | **Blockchair `raw/block` 暴露的节点 `valuePools`** | ✅ **节点自身记账项，14 个时点逐块读取** |
| **链上总供应 `chainSupply`** | 同上 | ✅ **与 CoinGecko 差 0.01%** |
| **区块补贴 1.5625 与减半点** | 同上，逐块采样定位 | ✅ **本报告测量** |
| **`lockbox` 开发基金余额与 6 万枚提取** | 同上 | ✅ **本报告测量** |
...
>
> **① 自查有用，但它只能抓清单上的东西。**
> 本期自查抓到的四处，**全部是前十期已经写下过的失败模式的复现**——
> 说明那份清单是有效的，也说明**同一个作者会重复犯同一类错误**。
> **而第 3 层抓到的三条，没有一条在清单上。**
>
> **② 本期最丢人的一处是内部矛盾**：把「谷底」写成 26.09%，
> **而正确答案（25.49%）就在同一张表往下两行。**
> **它不需要任何外部知识就能被发现，而作者没发现，脚本核查也不检查这个。**
> **建议：凡是文中出现「峰值」「谷底」「最高」「最低」这类极值断言，
> 都必须回到数据表里用 min/max 复算一次——这是一个可以机械化的检查。**
